...
def main():
    ...
    with KuavoEnv(
        ...
        output_dir=output,
            ...
            output_video_path = f'./{cfg.task.name}_{current_time}_speedx1.mp4'
            stop_event = threading.Event()  
            video_thread = threading.Thread(target=env.record_video, args=(output_video_path, 640, 480, 30, True,stop_event))
            video_thread.start()
            ...
            while True:
                ...
                try:
                    ...
                    import imageio
                    current_time = time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())
                    output_video_path = f'./{cfg.task.name}_{current_time}_speedx.mp4'
                    ...
                    while True:
                        ...
                        # ax.set_title("Concatenated Image")
                        # plt.pause(0.1)  # 非阻塞刷新
                        ...
                        print("\n\n")
                        env.exec_actions(
                            actions=action[:8],cur_state=obs_dict_np['agent_pos'][-1][:6]
                        )
